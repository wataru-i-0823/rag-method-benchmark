...
プロファイルは`work/src/rag_lab/profiles.py`で定義し、`--profile`で選択する。

| プロファイル | 埋め込み経路 | 保存ルール | 想定用途 |
| --- | --- | --- | --- |
| `local` | 決定論的なCPUベースラインである`chroma_hash` | `work/data/chroma/`はローカル保存・Git追跡外 | パイプラインの学習とGPU不要な手法比較 |
...
新しいプロファイルを追加する場合は、`work/src/rag_lab/profiles.py`でデフォルト手法、Chromaのパス、埋め込みデバイスを定義し、`work/README.md`へ記載する。APIキーや外部モデルのダウンロードを必要としないプロファイル振り分けテストも追加する。
